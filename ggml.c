...
#define GGML_F32x4_REDUCE(res, x0, x1, x2, x3) res = vaddvq_f32(vaddq_f32(vaddq_f32(x0, x1), vaddq_f32(x2, x3)))
...
#define GGML_F32_VEC_REDUCE GGML_F32x4_REDUCE
    ...
    #define GGML_F16x8_REDUCE(res, x0, x1, x2, x3)        \
    {                                                     \
        x0 = vaddq_f16(x0, x1);                           \
        x2 = vaddq_f16(x2, x3);                           \
        x0 = vaddq_f16(x0, x2);                           \
        float32x4_t t0 = vcvt_f32_f16(vget_low_f16 (x0)); \
        float32x4_t t1 = vcvt_f32_f16(vget_high_f16(x0)); \
        t0  = vaddq_f32 (t0, t1);                         \
        res = vaddvq_f32(t0);                             \
    }
    ...
    #define GGML_F16_EPS  4

    #define GGML_F16x4              float32x4_t
    #define GGML_F16x4_ZERO         vdupq_n_f32(0.0f)
    #define GGML_F16x4_SET1(x)      vdupq_n_f32(x)
    #define GGML_F16x4_LOAD(x)      vcvt_f32_f16(vld1_f16(x))
    #define GGML_F16x4_STORE(x, y)  vst1_f16(x, vcvt_f16_f32(y))
    #define GGML_F16x4_FMA(a, b, c) vfmaq_f32(a, b, c)
    #define GGML_F16x4_ADD          vaddq_f32
    #define GGML_F16x4_MUL          vmulq_f32
    #define GGML_F16x4_REDUCE(res, x0, x1, x2, x3) res = vaddvq_f32(vaddq_f32(vaddq_f32(x0, x1), vaddq_f32(x2, x3)))
...
#if defined(__FMA__)
    #define GGML_F32x8_FMA(a, b, c) _mm256_fmadd_ps(a, b, c)
#else
    #define GGML_F32x8_FMA(a, b, c) _mm256_add_ps(_mm256_mul_ps(a, b), c)
#endif
#define GGML_F32x8_ADD          _mm256_add_ps
#define GGML_F32x8_MUL          _mm256_mul_ps
#define GGML_F32x8_REDUCE(res, x0, x1, x2, x3)                                                \
{                                                                                             \
    x0 = _mm256_add_ps(x0, x1);                                                               \
    x2 = _mm256_add_ps(x2, x3);                                                               \
    x0 = _mm256_add_ps(x0, x2);                                                               \
    const __m128 t0 = _mm_add_ps(_mm256_extractf128_ps(x0, 0), _mm256_extractf128_ps(x0, 1)); \
    const __m128 t1 = _mm_movehl_ps(t0, t0);                                                  \
    res = _mm_cvtss_f32(_mm_add_ss(t0, t1));                                                  \
}
...
#define GGML_F16x8              __m256
#define GGML_F16x8_ZERO         _mm256_setzero_ps()
#define GGML_F16x8_SET1(x)      _mm256_set1_ps(x)
#define GGML_F16x8_LOAD(x)      _mm256_cvtph_ps(_mm_loadu_si128((__m128i*)(x)))
#define GGML_F16x8_STORE(x, y)  _mm_storeu_si128((__m128i*)(x), _mm256_cvtps_ph(y, 0))
#define GGML_F16x8_FMA          GGML_F32x8_FMA
#define GGML_F16x8_ADD          _mm256_add_ps
#define GGML_F16x8_MUL          _mm256_mul_ps
#define GGML_F16x8_REDUCE       GGML_F32x8_REDUCE
...
#define GGML_F32x4_REDUCE(res, x0, x1, x2, x3) \
{                                              \
    x0 = wasm_f32x4_add(x0, x1);               \
    x2 = wasm_f32x4_add(x2, x3);               \
    x0 = wasm_f32x4_add(x0, x2);               \
    res = wasm_f32x4_extract_lane(x0, 0) +     \
          wasm_f32x4_extract_lane(x0, 1) +     \
          wasm_f32x4_extract_lane(x0, 2) +     \
          wasm_f32x4_extract_lane(x0, 3);      \
}
...
inline static void ggml_vec_dot_f32(const int n, float * restrict s, const float * restrict x, const float * restrict y) {
    ...
    GGML_F32_VEC x0, x1, x2, x3;
    GGML_F32_VEC y0, y1, y2, y3;

    for (int i = 0; i < np; i += GGML_F32_STEP) {
        x0 = GGML_F32_VEC_LOAD(x + i + 0*GGML_F32_EPS);
        x1 = GGML_F32_VEC_LOAD(x + i + 1*GGML_F32_EPS);
        x2 = GGML_F32_VEC_LOAD(x + i + 2*GGML_F32_EPS);
        x3 = GGML_F32_VEC_LOAD(x + i + 3*GGML_F32_EPS);

        y0 = GGML_F32_VEC_LOAD(y + i + 0*GGML_F32_EPS);
        y1 = GGML_F32_VEC_LOAD(y + i + 1*GGML_F32_EPS);
        y2 = GGML_F32_VEC_LOAD(y + i + 2*GGML_F32_EPS);
        y3 = GGML_F32_VEC_LOAD(y + i + 3*GGML_F32_EPS);

        sum0 = GGML_F32_VEC_FMA(sum0, x0, y0);
        sum1 = GGML_F32_VEC_FMA(sum1, x1, y1);
        sum2 = GGML_F32_VEC_FMA(sum2, x2, y2);
        sum3 = GGML_F32_VEC_FMA(sum3, x3, y3);
    }

    // reduce sum0..sum3 to sum0
    GGML_F32_VEC_REDUCE(sumf, sum0, sum1, sum2, sum3);
...
inline static void ggml_vec_dot_f16(const int n, float * restrict s, ggml_fp16_t * restrict x, ggml_fp16_t * restrict y) {
    ...
    GGML_F16_VEC x0, x1, x2, x3;
    GGML_F16_VEC y0, y1, y2, y3;

    for (int i = 0; i < np; i += GGML_F16_STEP) {
        x0 = GGML_F16_VEC_LOAD(x + i + 0*GGML_F16_EPS);
        x1 = GGML_F16_VEC_LOAD(x + i + 1*GGML_F16_EPS);
        x2 = GGML_F16_VEC_LOAD(x + i + 2*GGML_F16_EPS);
        x3 = GGML_F16_VEC_LOAD(x + i + 3*GGML_F16_EPS);

        y0 = GGML_F16_VEC_LOAD(y + i + 0*GGML_F16_EPS);
        y1 = GGML_F16_VEC_LOAD(y + i + 1*GGML_F16_EPS);
        y2 = GGML_F16_VEC_LOAD(y + i + 2*GGML_F16_EPS);
        y3 = GGML_F16_VEC_LOAD(y + i + 3*GGML_F16_EPS);

        sum0 = GGML_F16_VEC_FMA(sum0, x0, y0);
        sum1 = GGML_F16_VEC_FMA(sum1, x1, y1);
        sum2 = GGML_F16_VEC_FMA(sum2, x2, y2);
        sum3 = GGML_F16_VEC_FMA(sum3, x3, y3);
    }

    // reduce sum0..sum3 to sum0
    GGML_F16_VEC_REDUCE(sumf, sum0, sum1, sum2, sum3);
...
inline static void ggml_vec_mad_f32(const int n, float * restrict y, const float * restrict x, const float v) {
    ...
    for (int i = 0; i < np; i += GGML_F32_STEP) {
        ...
        y0 = GGML_F32_VEC_FMA(y0, x0, vx);
        y1 = GGML_F32_VEC_FMA(y1, x1, vx);
        y2 = GGML_F32_VEC_FMA(y2, x2, vx);
        y3 = GGML_F32_VEC_FMA(y3, x3, vx);

        GGML_F32_VEC_STORE(y + i + 0*GGML_F32_EPS, y0);
        GGML_F32_VEC_STORE(y + i + 1*GGML_F32_EPS, y1);
        GGML_F32_VEC_STORE(y + i + 2*GGML_F32_EPS, y2);
        GGML_F32_VEC_STORE(y + i + 3*GGML_F32_EPS, y3);
    }
...
inline static void ggml_vec_mad_f16(const int n, ggml_fp16_t * restrict y, ggml_fp16_t * restrict x, const float v) {
    ...
    for (int i = 0; i < np; i += GGML_F16_STEP) {
        ...
        y0 = GGML_F16_VEC_FMA(y0, x0, vx);
        y1 = GGML_F16_VEC_FMA(y1, x1, vx);
        y2 = GGML_F16_VEC_FMA(y2, x2, vx);
        y3 = GGML_F16_VEC_FMA(y3, x3, vx);

        GGML_F16_VEC_STORE(y + i + 0*GGML_F16_EPS, y0);
        GGML_F16_VEC_STORE(y + i + 1*GGML_F16_EPS, y1);
        GGML_F16_VEC_STORE(y + i + 2*GGML_F16_EPS, y2);
        GGML_F16_VEC_STORE(y + i + 3*GGML_F16_EPS, y3);
    }
...
inline static void ggml_vec_scale_f32(const int n, float * y, const float   v) {
    ...
    for (int i = 0; i < np; i += GGML_F32_STEP) {
        ...
        y0 = GGML_F32_VEC_MUL(y0, vx);
        y1 = GGML_F32_VEC_MUL(y1, vx);
        y2 = GGML_F32_VEC_MUL(y2, vx);
        y3 = GGML_F32_VEC_MUL(y3, vx);
